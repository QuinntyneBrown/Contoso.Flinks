param (
    $resourceBaseName="contoso.Flinks$( Get-Random -Maximum 1000)",
    $location='eastus'
)
...
dotnet build ..\src\Contoso.Flinks.Api\Contoso.Flinks.Api.csproj

Write-Host 'Packaging API project for deployment' -ForegroundColor Green
dotnet publish ..\src\Contoso.Flinks.Api\Contoso.Flinks.Api.csproj --self-contained -r win-x86 -o ..\publish
